    ...
    <View style={styles.card}>
      <View style={styles.cardHeader}>
        <Text bold style={styles.textHeader}>
          {item?.created_at
            && moment.unix(item.created_at).format("DD/MM/YYYY HH:mm:ss") || 'Unknown'
          }
        </Text>
        {renderStatus(parseInt(item?.status))}
      </View>
      <Text style={styles.url}>{item?.url}</Text>
      <Text>{JSON.stringify(item?.data)}</Text>
      {item?.body && <Text style={styles.body}>{JSON.stringify(item?.body)}</Text>}
    </View>
  )
